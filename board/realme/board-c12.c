...
static const char *get_sbc_status(void) {
    uint32_t *sbc_reg = (uint32_t*)0x11c50060;
    uint32_t value = *sbc_reg;
    uint32_t sbc_enabled = (value << 0x1E) >> 0x1F;
    
    return sbc_enabled ? "enabled" : "disabled";
}
